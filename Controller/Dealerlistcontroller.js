...
import DealerListSchema from "../Models/DealerListmodel.js";
...
const adddealerlist = async (req, res) => {
  ...
  try {
    ...
    let dealerList = await DealerListSchema.findOne({ clientId });

    if (!dealerList) {
      // If it doesn't exist, create a new one
      dealerList = new DealerListSchema({ clientId, tyres });
    } else {
  ...
  const getDealerList = async (req, res) => {
    try {
      const dealerLists = await DealerListSchema.aggregate([
        {
          $lookup: {
